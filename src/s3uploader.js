...
function getUploadParameters(inputData) {  
  const base64Data = new Buffer.from(inputData.replace(/^data:image\/\w+;base64,/, ""), 'base64');
...
module.exports = {
    uploadImage: async function(fileData) {
      ...
      const shareUrl = await s3Link.getSignedUrl(s3Client, command, { expiresIn: process.env.spaces_expires_in });
